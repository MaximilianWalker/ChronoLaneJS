...
and any known mitigations. Maintainers will keep triage and remediation inside
the private GitHub security advisory until a coordinated release is available.
...
Maintainers handle a confirmed vulnerability through a draft GitHub security
...
access temporary private forks, so maintainers run the repository's required
validation locally while the fix remains confidential.

When the fix is ready, maintainers base its release-bearing `fix:` commit on
`main` and use GitHub's security-advisory merge control to land it in the public
repository. The resulting `main` push runs the gated release workflow, which
validates and publishes the patched npm version. The advisory is published
after the patched package is available. `main` is then merged back into `dev`
so ongoing development contains the fix and both branches retain shared
ancestry.
...
release cannot be produced safely, maintainers publish mitigation guidance in
the advisory rather than moving the report to a public issue prematurely.

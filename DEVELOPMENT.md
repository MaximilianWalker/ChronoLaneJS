...
every CI job to pass against the current branch tip before it can be merged.
Each merge to `main` triggers a second validation of the exact merge commit and
deploys that commit to GitHub Pages. The gated Release workflow runs from the
same push, but publishes to npm only when releases are enabled and
semantic-release finds a release-bearing Conventional Commit.
